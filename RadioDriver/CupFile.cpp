...
void CupFile::read(FsFile& file, Settings& settings) {
  ...
  while (file.fgets(line, maxlen) > 0) {
    ...
    else if (strcmp(label, "activeFrequency") == 0) {
      settings.ActiveFrequency = atol(value);
    }

    else if (strcmp(label, "activeName") == 0) {
      int here = maxlen-1;
      while(isspace(line[here])){
        line[here] = 0;
        --here;
      }
      strncpy(settings.ActiveName, value, sizeof(Settings::ActiveName));
      settings.ActiveName[sizeof(Settings::ActiveName)-1] = 0;
    }

    else if (strcmp(label, "standbyFrequency") == 0) {
      settings.StandbyFrequency = atol(value);
    }

    else if (strcmp(label, "standbyName") == 0) {
      int here = maxlen-1;
      while(isspace(line[here])){
        line[here] = 0;
        --here;
      }
      strncpy(settings.StandbyName, value, sizeof(Settings::StandbyName));
      settings.StandbyName[sizeof(Settings::StandbyName)-1] = 0;
    }
  }
}
